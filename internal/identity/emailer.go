...
// GenerateOTP crée un code à 6 chiffres
func GenerateOTP() string {
	r := rand.New(rand.NewSource(time.Now().UnixNano()))
	return fmt.Sprintf("%06d", r.Intn(1000000))
}

// SendOTPEmail envoie le mail de façon synchrone (sera appelé dans une goroutine)
func SendOTPEmail(toEmail, otpCode string) error {
	auth := smtp.PlainAuth("", SenderEmail, SenderAppPwd, SMTPHost)
	subject := "Subject: Your CloudSim OTP Code\n"
	mime := "MIME-version: 1.0;\nContent-Type: text/plain; charset=\"UTF-8\";\n\n"
	body := fmt.Sprintf("Hello,\n\nYour OTP code for CloudSim is: %s\n\nThis code expires in 5 minutes.", otpCode)
